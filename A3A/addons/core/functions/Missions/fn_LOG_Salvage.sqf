...
private _typeVeh = if (_difficultX) then { selectRandom (GetTiered(_faction, "vehiclesGunBoats")) } else { selectRandom (GetTiered(_faction, "vehiclesTransportBoats")) };
...
if (_typeSDV != "") then {
   ...
   _typeSDV = selectRandom (GetTiered(_faction, "vehiclesSDV"));
