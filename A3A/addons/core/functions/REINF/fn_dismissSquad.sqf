	...
	_groupX = _x;
	{
		if (alive _x) then
		{
			...
			if (_backpck != "") then
			{
				...
				switch (true) do
				{
					case (_assemblesTo in (FactionGetAll(reb, "staticMortars"))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
					case (_assemblesTo in (FactionGetAll(reb, "staticAA"))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
					case (_assemblesTo in (FactionGetAll(reb, "staticMGs"))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
					case (_assemblesTo in (FactionGetAll(reb, "staticAT"))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
				};
			};
		};
		deleteVehicle _x;
