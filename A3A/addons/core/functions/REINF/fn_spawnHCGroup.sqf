...
switch _special do {
    ...
    case "staticAutoT": {
        private _staticType = switch _idFormat do {
            case "Mort-": {(FactionGetAll(reb, "staticMortars")) # 0};  
            case "MG-": {(FactionGetAll(reb, "staticMGs")) # 0};
            default {""};
        };
    ...
    //vehicle squad
    case "BuildAA": {
        private _static = ((attachedObjects _vehicle) select {typeOf _x in FactionGetAll(reb, "staticAA")})#0;
        (_units # (_countUnits -1)) moveInDriver _vehicle;
        (_units # _countUnits) moveInGunner _static;
        call _initVeh;
        _vehicle allowCrewInImmobile true;
        _cost = _cost + ([(FactionGetAll(reb, "staticAA")) # 0] call A3A_fnc_vehiclePrice);
    };
    case "VehicleSquad": {
        (_units # (_countUnits -1)) moveInDriver _vehicle;
    ...
    call _initInfVeh;
    case "MG": {
        private _backpacks = getArray (configFile/"CfgVehicles"/(FactionGetAll(reb, "staticMGs")) # 0/"assembleInfo"/"dissasembleTo");
        (_units # (_countUnits - 1)) addBackpackGlobal (_backpacks#1);
        (_units # _countUnits) addBackpackGlobal (_backpacks#0);
        _cost = _cost + ([(FactionGetAll(reb, "staticMGs")) # 0] call A3A_fnc_vehiclePrice);
    };
    case "Mortar": {
        private _backpacks = getArray (configFile/"CfgVehicles"/(FactionGetAll(reb, "staticMortars")) # 0/"assembleInfo"/"dissasembleTo");
        (_units # (_countUnits - 1)) addBackpackGlobal (_backpacks#1);
        (_units # _countUnits) addBackpackGlobal (_backpacks#0);
        _cost = _cost + ([(FactionGetAll(reb, "staticMortars")) # 0] call A3A_fnc_vehiclePrice);
    };
};
